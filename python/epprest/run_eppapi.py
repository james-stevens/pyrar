...
if not os.path.isfile(static.LOGINS_FILE):
    raise ValueError(f"Logins file `{static.LOGINS_FILE}` is missing")

with open(static.LOGINS_FILE, "r") as fd:
    logins = json.load(fd)

this_reg = os.environ["PYRAR_REGISTRY"]
if this_reg not in logins:
    raise ValueError(f"Registry '{this_reg}' not in '{static.LOGINS_FILE}'")
