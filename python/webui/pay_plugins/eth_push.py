...
def eth_push_html(user_id):
    ...
    html = """<table align=center cellspacing=1 cellpadding=0 border=0>
        ...
        <tr>
            <td class=formPrompt>Etherium wallet you will send payments from :</td>
        ...
        <input type=hidden id="pay.provider" value="eth_push">
        <tr><td colspan=2><div style='height: 15px;'></div></td></tr>
        <tr><td colspan=2 class=btmBtnBar><input
            ...
            type=button
            style='width: 175px;'
            value="Add Etherium Push"></td></tr>
        </table>"""
    return True, html


def eth_push_validate(data):
    data["single_use"] = 0
    data["can_pull"] = 0
    data["provider"] = "eth_push"
    return True
